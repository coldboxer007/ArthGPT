      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
        ...
        {chatHistory.map((msg, i) => (
          <div key={i} className={cn('flex', msg.role === 'user' ? 'justify-end' : 'justify-start')}>
            {msg.role === 'user' ? (
              <div className="max-w-[85%] px-4 py-3 bg-amber-600 rounded-2xl rounded-tr-sm shadow-md">
              ...
              <div className="max-w-[90%] bg-navy-800 border border-navy-700 rounded-2xl rounded-tl-sm p-4 shadow-lg relative overflow-hidden">
                ...
                {msg.pipeline && msg.intentType !== 'advisory' && msg.intentType !== 'unknown' && (
                  <button
                    ...
                    className="mt-3 w-full flex items-center justify-center gap-2 px-3 py-2 text-xs font-semibold text-amber-400 bg-amber-500/10 border border-amber-500/20 rounded-xl hover:bg-amber-500/20 transition-all duration-200"
                  >
      ...
      <form onSubmit={handleOrchestratorSubmit} className="shrink-0 border-t border-navy-800 bg-navy-950 p-4">
        <div className="relative flex items-center">
          <input
            ...
            className="w-full pl-4 pr-12 py-3.5 bg-navy-900 border border-navy-700 rounded-2xl text-sm text-white placeholder-slate-500 focus:outline-none focus:border-amber-500/50 focus:ring-1 focus:ring-amber-500/30 transition-all disabled:opacity-50 shadow-inner"
          />
          <button
            type="submit"
            disabled={orchestratorLoading || !orchestratorQuery.trim()}
            className="absolute right-2 p-2 bg-amber-500 text-navy-950 rounded-xl hover:bg-amber-400 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-md"
          >
